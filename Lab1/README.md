...
4. Zrobiłem/-am getter i mam błąd kompilacji.
   - Gettery z założenia nic nie modyfikują, stąd getter powinien być metodą stałą - zawierającą słówko `const` po prawej stronie
5. Mam błąd kompilacji z wywołaniem metody `print()`, który zawiera słówko `const`
   - Metoda `print()` ma nic nie zmieniać w zawartości klasy, stąd powinna być opatrzona przydomkiem `const` po prawej stronie (metoda stała)
...
2. Warto konstruktor zawierający argumenty domyślne poprzedzić słówkiem `explicite`
3. Warto gettery poprzedzić artybutem `[[nodiscard]]`
4. Odsyłam do [wytycznych jak pisać porządny kod](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines)
